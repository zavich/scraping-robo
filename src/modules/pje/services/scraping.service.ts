...
export class NewScrapingService {
  ...
  async execute(processNumber: string, regionTRT: number, instance: number) {
    ...
    if (wafParams?.websiteKey && wafParams?.context && wafParams?.iv) {
      ...
      if (!tokenToUse) {
        throw new Error('existing_token não retornado pelo resolvedor AWS WAF');
      }

      //
      // 3. OBTER /voucher DO WAF
      //
      const voucherBaseUrl = (wafParamsExtracted.challengeScript || '').replace(
        /\/challenge\.js$/,
        '',
      );
      ...
      try {
        ...
        const wafOriginal = originalCookies.find((c) => c.name.includes('aws'));
